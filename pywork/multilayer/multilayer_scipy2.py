...
Area = 90.7496993       #56m3内容器外表面积 m2
Qgls1 = 27.99159        #前支撑玻璃钢漏热
Qgls2 = 29.45           #后支撑玻璃钢漏热
Qpip = 3.979            #管路漏热
...
class Multilayer(object):
    def __init__(self, num):
        self.tlayer0 = 300           # 外壳温度
        self.tlayer1 = 299.9         # 外壳温度
        self.tlayern = 111           # 内壁温度
        self.qrad = []               # 辐射传热
        self.qsld = []               # 固体传热
        self.qgas = []               # 气体传热
        self.qtot = []               # 总传热
        ...
        self.qrad.append(float(A1 * (self.tlayer0**4 - self.tttt[0]**4)))
        self.qsld.append(float(A2 * (self.tlayer0 - self.tttt[0])))
        self.qgas.append(float(A3 * (self.tlayer0 - self.tttt[0])))
        ...
        for jj in range(layer_n - 1):
            ...
            self.qtot.append(float(self.qrad[jj+1] + self.qsld[jj+1] + self.qgas[jj+1]))

        self.qsld[0] = self.qtot[1]

        self.lamda = self.q0 * (layer_n) * delta_x / (self.tlayer0-self.tlayern)
        self.Qmti = self.q0 * Area
        self.Qtot = self.Qmti + Qgls1 + Qgls2 + Qpip
        self.tht1 = 24 * 3600 * self.Qtot / 56 / 430 / 510.2 /1000 * 100
        self.tht2 = 24 * 3600 * self.Qtot * 196 / 162 / 56 / 810 / 197.6 /1000 * 100

    def diedai(self, _tlayer1):
        layer_n = self.layer + 1
        self.tttt = []
        for ii in range(layer_n):
                if ii == 0:
                    self.q0 = A1 * (self.tlayer0**4 - _tlayer1**4) + (A2 + A3) * (self.tlayer0 - _tlayer1)
                    self.tttt.append(_tlayer1)
                if ii >= 1:
                    self.tttt.append(solver(self.tttt[ii-1], self.q0))
                    self.solver_count += 1

    def diedai2(self, _tlayer1):
        layer_n = self.layer + 1
        self.tttt = []
        for ii in range(layer_n):
                if ii == 0:
                    self.tttt.append(self.tlayer0)
                if ii == 1:
                    self.q0 = A1 * (self.tlayer0**4 - _tlayer1**4) + (A2 + A3) * (self.tlayer0 - _tlayer1)
                    self.tttt.append(_tlayer1)
                if ii >= 2:
    ...
    def plt(self):
        ...
        pyplot.axis([0, self.layer, 0, self.qtot[1] * 1.05])
...
# pyplot.xlabel(u'层数')
# pyplot.ylabel(u'温度(k)')
# pyplot.axis([0, ml50.layer, 0, ml50.tlayer0 * 1.1])
# pyplot.plot(range(ml30.layer + 1), ml30.tttt, '--', label=u'30层', linewidth=1.5)
# pyplot.plot(range(ml40.layer + 1), ml40.tttt, '-.', label=u'40层', linewidth=1.5)
# pyplot.plot(range(ml50.layer + 1), ml50.tttt, '-', label=u'50层', linewidth=1.5)
# pyplot.legend(loc='upper right')
#
# pyplot.figure(2, figsize=(6, 5))
# pyplot.xlabel(u'层数')
